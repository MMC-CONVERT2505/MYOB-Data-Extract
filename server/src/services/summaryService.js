 ...
 * signed-in user.
 *
 * ── "Total Lines" — confirmed source (client/src/pages/Summary.jsx:263) ──
 *   <Row label="Total Lines" value={t.totalGLLines ?? t.totalLines} strong />
 *
 * The frontend displays `totalGLLines`, falling back to `totalLines` only
 * if `totalGLLines` is missing. `totalGLLines` is nowhere near expensive
 * to compute — it's built while walking `journalTxnInRange`, which is
 * ALREADY fetched for the bank/invoice/bill GL-line breakdowns below, so
 * it costs nothing extra.
 *
 * Two earlier attempts to "speed this up" both got this wrong:
 *   1. Sourcing totalLines from /Report/JournalTransactionHistory's
 *      Count field — that number (e.g. 43,966) matches NEITHER
 *      totalLines NOR totalGLLines; it's some other, more granular MYOB
 *      report count and isn't a valid substitute for either metric here.
 *   2. Restoring the invoice/bill per-record detail fetch
 *      (fetchInvoiceDetail/fetchBillDetail via fetchDetailsInBatches) to
 *      make totalLines "accurate" — pointless, since totalLines is never
 *      actually rendered when totalGLLines is present (which it always
 *      is). That N+1 loop was the single biggest cost in the whole
 *      summary and bought nothing.
 *
 * Both have been removed. totalGLLines is computed locally (free) with
 * the original `+50` buffer, and totalLines is simply aliased to the same
 * value — correct, and with zero extra API calls.
 */
...
/**
 * Runs `worker(item)` over `items` with up to `concurrency` requests in
 * flight at once, as a CONTINUOUS pool — not lockstep batches. Each
 * worker immediately pulls the next item off the queue as soon as it
 * finishes, instead of waiting for 3 siblings in the same "batch" to
 * finish first (the old bill-attachment code's batches-of-4 pattern).
 *
 * Rate-limit handling is SHARED and ADAPTIVE instead of a blind fixed
 * pause after every batch: if any worker gets a 429, every worker
 * respects a shared cooldown before its next pull. If MYOB never
 * rate-limits you, no delay is ever added — the old code paused 400ms
 * after every single batch regardless of whether it was needed, which
 * on ~250 batches alone cost ~100 seconds for nothing.
 */
async function runPool(items, worker, { concurrency = 8, cooldownMs = 2000 } = {}) {
  ...
  if (rateLimitHits > 0) {
    console.log(`⚠️ runPool: hit 429 ${rateLimitHits} time(s), applied ${cooldownMs}ms shared cooldown each time.`);
  }
...
export async function getTransactionCounts(dbUser, userId, { startDate, endDate, accounts }) {
  ...
  // ── GL-posting-level line counts (JournalTransaction-based, matches MYOB desktop report) ──
  // This is the source of `totalGLLines`, i.e. the field actually rendered
  // as "Total Lines" on the frontend — see file header. It costs nothing
  // extra: journalTxnInRange is already fetched above for the breakdowns
  // below (bank/cc, invoice/credit-memo, bill/bill-credit GL-line splits).
  const glLineCounts = journalTxnInRange.reduce(
    (acc, jt) => {
      const key = jt.JournalType || "Unknown";
      const lines = Array.isArray(jt.Lines) ? jt.Lines.length : 0;
  ...
  // ── Total GL lines (= the "Total Lines" the frontend actually shows) ──
  // Computed locally from journalTxnInRange — already fetched above, no
  // extra API call needed. `+50` buffer preserved from the original.
  const LINE_BUFFER = 50;
  const totalGLLines = glLineCounts.totalGLLines + LINE_BUFFER;

  // `totalLines` kept in the response only as the fallback the frontend
  // checks (`t.totalGLLines ?? t.totalLines`); aliased to the same value
  // since totalGLLines is always populated here. No per-invoice/per-bill
  // detail fetch — that N+1 loop was removed entirely (see file header).
  const totalLines = totalGLLines;
  ...
  console.log(
    `📊 Summary [${dateOnly(startDate)}..${dateOnly(endDate)}] ` +
      ...
      `cm=${creditMemoRecords.length} bill=${billRecords.length} ` +
      `billCr=${billCreditRecords.length} journal=${journalsInRange.length} ` +
      `totalGLLines=${totalGLLines} (local, journalTxnInRange sum + ${LINE_BUFFER} buffer)`
  );
