...
import { flattenMYOBRawData } from "./converters/myobRawData.js";
import { flattenMYOBRawDataToSheets } from "./converters/myobRawDataSheets.js";
...
// True raw flatten: every field from the MYOB API response, no
// dataType/subType-specific field selection, nothing dropped.
// NOTE: kept as-is (single flat array) for any caller still using the
// single-sheet shape. The Excel-generation paths (sync + async) now
// use convertToMYOBRawDataSheets below instead, which fixes the
// column-limit corruption this single-sheet shape could hit.
export const convertToMYOBRawData = (items) => {
  if (!items?.length) return [];
  return flattenMYOBRawData(items);
};

// ── MYOB Raw DATA Converter — multi-sheet (safe for Excel) ───────
// Same "every field preserved, nothing dropped" guarantee as above,
// but structurally cannot exceed Excel's 16,384-column limit: any
// array field large enough to have caused that (Lines[], a payment's
// Invoices[]/Bills[], etc.) is moved to its own child sheet instead of
// flattened into thousands of columns. Small/bounded arrays (e.g. a
// Contact's Addresses[], capped at 5 by MYOB) stay flattened inline,
// unchanged from today. See converters/myobRawDataSheets.js for the
// full design.
export const convertToMYOBRawDataSheets = (items, rootSheetName) => {
  if (!items?.length) return { sheets: [{ name: rootSheetName || "Data", rows: [] }] };
  return flattenMYOBRawDataToSheets(items, rootSheetName);
};
